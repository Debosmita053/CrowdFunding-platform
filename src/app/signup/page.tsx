...
export default function SignUpPage() {
  ...
  const handleSubmit = async () => {
    ...
    if (!formData.termsAccepted || !formData.blockchainConsent) {
      alert('Please accept the terms and conditions and blockchain consent');
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (errorMessage.includes('Database not configured')) {
          throw new Error('Database setup required. Please check the console for setup instructions.');
        }
        throw new Error(errorMessage);
      }

      alert('Account created successfully! You can now explore campaigns.');
  ...
  const canProceed = () => {
    switch (currentStep) {
      case 1:
        return formData.fullName.trim() && formData.email.trim() && formData.password && 
               formData.confirmPassword === formData.password && passwordStrength >= 3;
      case 2:
        return formData.role && otpData.emailVerified;
      case 3:
        return isConnected;
      case 4:
